...
#                  options are: 'ranking_proportional',...
...
def parent_selection(method, population, n, N, lbd, s):
    ...
    if method=='ranking_proportional':
        ...
        sorted_indices=np.argsort(current_population_fitness)
        weights = np.asarray([(l+1) for l in range(len(current_population_fitness)-1)])
        parents_indices = np.append(sorted_indices[0],np.random.choice(sorted_indices[1:], size=n-1, replace=False, p=weights/sum(weights)))
